...
1. **Contact form**: the form on `contact.html` submits in the background via `fetch()` (see the inline `<script>` right after the form) to a Google Apps Script Web App (`script.google.com/macros/s/.../exec`), bound to a Google Sheet ("Maple Garden — Contact Enquiries") owned by `maplegardenwellbeing@gmail.com`. On submit the script appends a row to the Sheet (Timestamp, Name, Email, Phone, Treatment, Message) and emails Andrea a copy via `MailApp.sendEmail`. The page then navigates itself (same-origin, no `fetch` involved) to `contact.html?sent=1`, which swaps the form for a thank-you message. The Sheet is the source of truth if an email notification ever fails to arrive — check it directly if an enquiry seems to have gone missing. A honeypot field (`_honey`) silently drops bot submissions without logging or emailing them. The `<form>`'s `action`/`method` attributes are kept only as a no-JS fallback (a real POST navigation to the exec URL, landing on Google's own Apps Script response page) — the normal path never navigates there, which avoids both Google's "created by a Google Apps Script user" banner and a sandboxed-iframe quirk where an automatic redirect out of that page can't reliably reach the top-level browser tab. The script itself lives in the Sheet's Extensions → Apps Script editor, not in this repo's build — `google-apps-script.gs` at the repo root is a manually-kept-in-sync copy for version history/reference only. To change the live behaviour: edit in the Apps Script editor (and update `google-apps-script.gs` to match), then **Deploy → Manage deployments → edit → New version** (saving alone does not update the live URL).
