...
1. **Contact form**: the form on `contact.html` posts to a Google Apps Script Web App (`script.google.com/macros/s/.../exec`), bound to a Google Sheet ("Maple Garden — Contact Enquiries") owned by `maplegardenwellbeing@gmail.com`. On submit it appends a row to the Sheet (Timestamp, Name, Email, Phone, Treatment, Message) and emails Andrea a copy via `MailApp.sendEmail`, then redirects back to `contact.html?sent=1`, which swaps the form for a thank-you message. The Sheet is the source of truth if an email notification ever fails to arrive — check it directly if an enquiry seems to have gone missing. A honeypot field (`_honey`) silently drops bot submissions without logging or emailing them. The script itself lives in the Sheet's Extensions → Apps Script editor, not in this repo's build — `google-apps-script.gs` at the repo root is a manually-kept-in-sync copy for version history/reference only. To change the live behaviour: edit in the Apps Script editor (and update `google-apps-script.gs` to match), then **Deploy → Manage deployments → edit → New version** (saving alone does not update the live URL).
